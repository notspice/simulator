...
pub const Gate = struct {
    ...
    pub fn init(gate_type: GateType, input_a_node: ?*Node, input_b_node: ?*Node) Gate {
        return Gate{ .gate_type = gate_type, .input_a_node = input_a_node, .input_b_node = input_b_node };
    }
};
...
pub const Simulator = struct {
    ...
    pub fn init(text_netlist: [*:0]const u8, alloc: std.mem.Allocator) !Simulator {
        var simulator: Simulator = .{ .nodes = std.AutoHashMap(usize, Node).init(alloc), .gates = std.ArrayList(Gate).init(alloc) };
